...
describe("sitemap.xml の品質検証", { skip: !distExists ? "dist/ が未生成。npm run build を実行してください" : false }, () => {
  ...
  test("URL 数が index + 全製品ページ + 同一ブランド内ペア数と一致すること", () => {
    const products = JSON.parse(readFileSync(PRODUCTS_PATH, "utf8"));
    const brandCounts = {};
    for (const p of products) brandCounts[p.brand] = (brandCounts[p.brand] || 0) + 1;
    const intraBrandPairs = Object.values(brandCounts).reduce((s, c) => s + c * (c - 1) / 2, 0);
    const expected = 1 + products.length + intraBrandPairs;
    const locMatches = sitemapContent.match(/<loc>([^<]+)<\/loc>/g) || [];
    urls = locMatches.map(m => m.replace(/<\/?loc>/g, ""));
  ...
  test("index URL (トップページ) が 1 件含まれること", () => {
    const locMatches = sitemapContent.match(/<loc>([^<]+)<\/loc>/g) || [];
    urls = locMatches.map(m => m.replace(/<\/?loc>/g, ""));
    const indexUrls = urls.filter(u => !u.includes("/compare/") && !u.includes("/products/"));
